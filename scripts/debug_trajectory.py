...
from planning_ros_msgs.msg import RunTrajectoryActionFeedback, PlanTwoPointActionGoal
...
class CheckTrajectory():
    def __init__(self):
        ...
        odom_ln, = self.ax.plot([], [], 'bo', label="Odom")
        self.cmap = mpl.cm.Set1
        traj_ln = self.ax.add_collection(LineCollection([], cmap=self.cmap, alpha=.5))
        ...
        traj_ln2 = self.ax.add_collection(LineCollection([], cmap=self.cmap))
        ...
        traj_feedback_ln, = self.ax.plot([], [], 'k*', label="Calculated Position from Feedback")
        query_ln, = self.ax.plot([], [], 'yo', label="Planner query start")

        self.ln = [odom_ln, traj_ln, start_ln, goal_ln, seg_num_ln, traj_feedback_ln, traj_ln2, query_ln]
        self.ax.legend()
    ...
    def traj_tracker_spline_traj_cb(self, msg):
        positions, velocities = self.spline_traj_process(msg)
